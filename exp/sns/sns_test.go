...
func (s *S) TestPublish(c *C) {
	...
	c.Assert(req.Method, Equals, "GET")
	c.Assert(req.URL.Path, Equals, "/")
	c.Assert(strings.Contains(req.URL.RawQuery, "My-TargetArn"), Equals, true)
	c.Assert(req.Header["Date"], Not(Equals), "")
...
func (s *S) TestPublishErrorResponse(c *C) {
	...
	c.Assert(err, FitsTypeOf, sampleError)
	casted := err.(*sns.Error)
	c.Assert(casted.Code, Equals, "EndpointDisabled")
